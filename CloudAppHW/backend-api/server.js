...
async function initGemini() {
  ...
  const candidates = preferred ? [preferred] : ['gemini-3-flash-preview', 'gemini-2.5-flash', 'gemma-3-27b-it'];
...
app.post('/api/itineraries', verifyFirebaseToken, async (req, res) => {
  const { title, destination, start_date, end_date, short_description, detail_description, is_private } = req.body;
  ...
  try {
    ...
    const [traveller] = await pool.execute('SELECT id, membership_tier FROM travellers WHERE email = ?', [email]);
    ...
    const membershipTier = traveller[0].membership_tier || 'Free';

    // Free user limit: max 2 itineraries
    if (membershipTier !== 'Premium') {
      const [countResult] = await pool.execute('SELECT COUNT(*) as count FROM itineraries WHERE traveller_id = ?', [traveller_id]);
      if (countResult[0].count >= 2) {
        return res.status(403).send({ message: 'Free users can only create up to 2 trips. Please upgrade to Premium for unlimited trips.' });
      }
    }

    // Only Premium users can create private itineraries
    const actualIsPrivate = (membershipTier === 'Premium' && is_private === true) ? 1 : 0;

    const [result] = await pool.execute(
      'INSERT INTO itineraries (traveller_id, title, destination, start_date, end_date, short_description, detail_description, is_private) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [traveller_id, title, destination, start_date, end_date, short_description || '', detail_description || '', actualIsPrivate]
    );
    ...
    // Check disable AI switch
    const enableAi = req.body.enable_ai !== false; // default true if not present

    if (!enableAi) {
      log('INFO', '[ITINERARY CREATE] AI Disabled by user', { itineraryId });
      ai_status = 'disabled';
      // Do NOT create Firestore doc, so GET /ai will 404
    } else {
      try {
    ...
    if (enableAi && (!suggestion || ai_status === 'no_suggestion' || ai_status === 'queued' || ai_status === 'error')) {
      (async () => {
        try {
...
app.get('/api/itineraries/feed', async (req, res) => {
  try {
    // Simple "Newest" feed - exclude private itineraries
    const [rows] = await pool.query(
      `SELECT i.id, i.title, i.destination, i.start_date, i.end_date, i.short_description, 
                t.email AS traveller_email, t.name AS traveller_name
         FROM itineraries i 
    ...
    log('ERROR', '[FEED] server error', { error: String(error?.message || error) });
    res.status(500).send({ message: 'Server error retrieving feed.' });
  }
});
...
// Get itinerary count for a user (for Free tier limit check)
app.get('/api/itineraries/count/:email', async (req, res) => {
  const { email } = req.params;
  try {
    ...
    const [result] = await pool.execute('SELECT COUNT(*) as count FROM itineraries WHERE traveller_id = ?', [traveller[0].id]);
    res.send({ count: result[0].count });
  } catch (error) {
    log('ERROR', '[COUNT] server error', { error: String(error?.message || error) });
    res.status(500).send({ message: 'Server error retrieving itinerary count.' });
  }
});
...
app.post('/api/travellers/ensure', verifyFirebaseToken, async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute('SELECT id, membership_tier, is_admin FROM travellers WHERE email = ?', [email]);
    if (rows.length > 0) {
      return res.send({
        message: 'Exists',
        email,
    ...
    const [countResult] = await pool.execute('SELECT COUNT(*) as count FROM travellers');
    const isFirstUser = countResult[0].count === 0;

    // First user becomes admin automatically
    if (isFirstUser) {
      log('INFO', '[travellers/ensure] First user detected, granting admin privileges', { email });
      const [result] = await pool.execute(
        'INSERT INTO travellers (email, name, is_admin, membership_tier) VALUES (?, ?, TRUE, ?)',
        [email, name, 'Premium']
      );
      return res.status(201).send({
        ...
        is_admin: true,
        message: 'Created (First user - Admin)'
      });
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// 13.6) Admin & Membership
async function verifyAdmin(req, res, next) {
  if (!req.user || !req.user.email) return res.status(401).send({ message: 'Unauthorized' });
  try {
    const [rows] = await pool.execute('SELECT is_admin FROM travellers WHERE email = ?', [req.user.email]);
    ...
    log('ERROR', '[Auth] admin check fail', { error: String(e) });
    res.status(500).send({ message: 'Server error check admin' });
  }
}

app.get('/api/admin/users', verifyFirebaseToken, verifyAdmin, async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT id, email, name, membership_tier, is_admin FROM travellers');
    res.send(rows);
  } catch (e) {
    log('ERROR', '[Admin] get users fail', { error: String(e) });
    res.status(500).send({ message: 'Server error' });
  }
});

app.put('/api/admin/users/:id/level', verifyFirebaseToken, verifyAdmin, async (req, res) => {
  const { id } = req.params;
  const { level } = req.body;
  try {
    await pool.execute('UPDATE travellers SET membership_tier = ? WHERE id = ?', [level, id]);
    res.send({ message: 'User level updated', id, level });
  } catch (e) {
    log('ERROR', '[Admin] update level fail', { error: String(e) });
    res.status(500).send({ message: 'Server error' });
  }
});

app.post('/api/users/upgrade', verifyFirebaseToken, async (req, res) => {
  log('INFO', `[Upgrade Request] User ${req.user.email} requested upgrade.`);
  // In a real app, you might save this request to DB
  res.send({ message: 'Upgrade request received. Please contact admin.' });
});
